...
type
  mos65xx_reg* = enum
    MOS65XX_REG_INVALID = 0, MOS65XX_REG_ACC, ## /< accumulator
...
type
  mos65xx_insn* = enum
    ...
    MOS65XX_INS_TYA, MOS65XX_INS_ENDING ##  <-- mark the end of the list of instructions


## / Group of MOS65XX instructions

type
  mos65xx_group_type* = enum
...
type
  INNER_C_UNION_mos65xx_131* {.bycopy, union.} = object
    reg*: mos65xx_reg          ## /< register value for REG operand
    imm*: uint8_t              ## /< immediate value for IMM operand
    mem*: uint16_t             ## /< base/index/scale/disp value for MEM operand

  cs_mos65xx_op* {.bycopy.} = object
    `type`*: mos65xx_op_type   ## /< operand type
    ano_mos65xx_131*: INNER_C_UNION_mos65xx_131


## / The MOS65XX address mode and it's operands

type
  cs_mos65xx* {.bycopy.} = object
    am*: mos65xx_address_mode
    modifies_flags*: bool      ## / Number of operands of this instruction,
                        ## / or 0 when instruction has no operand.
    op_count*: uint8_t
    operands*: array[3, cs_mos65xx_op] ## /< operands for this instruction.
